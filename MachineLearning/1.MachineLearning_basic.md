...
__Placeholder__ (플레이스홀더) 란 계산 그래프에 데이터가 투입되는 위치를 나타내는 것이다. __Session__ 의 ```feed_dict``` 인자를 통해서 플레이스홀더에 데이터를 투입한다. 계산 그래프에 플레이스홀더를 넣기 위해서는 플레이스홀더에 대해 _하나 이상_ 의 연산을 수행해야 한다.
...
만약에 차원이 다양하거나 알 수 없는 경우를 처리하기 위해서는 ```None``` 으로 지정을 한다. 예를 들어 플레이스홀더의 열 크기를 알 수 없는 경우 다음과 같이 한다.
...
|:-----:|:----:|:-----|:-----|
|L1 Norm(절대 비용 함수)|회귀|좀 더 견고함|덜 안정적|
|L2 Norm(유클리드 비용 함수)|회귀|좀 더 안정적|덜 견고함|
