...
다음 예제는 _L2 Norm, L1 Norm, Pseudo-Huber_ 비용 함수들을 구현한 예제이다. ```x_vals``` 의 값을 -1과 1 사이의 500개의 숫자로 _예측 값_ 이라고 가정하고, ```target``` 을 _대상 값_ 이라고 지정하였을 때 각각 비용 함수들에 대한 결과이다. 결과는 그래프로 나타낸다. 위 세 개의 비용 함수들은 연속 종속 변수를 예측하는 __회귀 비용 함수__ 이다.
